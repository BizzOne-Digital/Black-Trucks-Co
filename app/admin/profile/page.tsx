...
import { Mail, Lock, CheckCircle, AlertCircle, Eye, EyeOff, Send } from 'lucide-react';
...
export default function AdminProfilePage() {
  ...
  const handleSmtpTest = async () => {
    ...
    try {
      ...
      setSmtpMsg({ type: 'success', text: data.message || 'Test email sent to blacktrucksco@gmail.com' });
    } catch (err: any) {
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-8 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Booking alerts are sent to <strong>blacktrucksco@gmail.com</strong>. Use this to verify Gmail SMTP is working.
          </p>

          {smtpMsg && (
            <div className={`flex items-center gap-2 text-sm rounded-lg px-4 py-3 mb-4 ${
              smtpMsg.type === 'success' ? 'bg-green-50 border border-green-200 text-green-700' : 'bg-red-50 border border-red-200 text-red-600'
            }`}>
              {smtpMsg.type === 'success' ? <CheckCircle className="h-4 w-4 flex-shrink-0" /> : <AlertCircle className="h-4 w-4 flex-shrink-0" />}
              {smtpMsg.text}
            </div>
          )}
          ...
          <button
            ...
            className="bg-black text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors inline-flex items-center gap-2"
          >
            <Send className="h-4 w-4" />
            {smtpLoading ? 'Sending test…' : 'Send test email'}
          </button>
        </div>
